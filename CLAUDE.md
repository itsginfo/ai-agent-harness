...
| **WoW AddOns** | `/Users/jamesmeirowsky/Projects/wow-addons/` | `/Users/jamesmeirowsky/Projects/wow-addons/CLAUDE.md` | GH Issues at `itsginfo/wow-addons` (gh auth via **itsginfo** — moved off the personal `JamesMeirowsky` account 2026-08-06 to end account drift; old repo superseded, its issues not migrated). Recurring reconciliation on `#2`; auto-best-gear build on `#1`. No Project board. Never used Monday. | CTO |
| **WoW Forever** | `/Users/jamesmeirowsky/Projects/wow-forever/` | `/Users/jamesmeirowsky/Projects/wow-forever/CLAUDE.md` | GH Issues at `itsginfo/wow-forever` (gh auth via **itsginfo**). Created 2026-09-17 (Forever beta day). Never used Monday. Reference library + addon-compatibility program against the Forever beta API. | PM (research/curation) / CTO (API analysis, addon ports) |
...
| Substantive new engineering work with design landscape → tracker | Entry fork: `/wayfinder` (>1 session or foggy → decision map) **or** `/grill-with-docs` (fits 1 session, clear) → `/to-spec` → `/to-tickets` → `/triage` → (build) → V-005/V-006 review | Don't run pipeline on **escape lanes:** Routine Requests (`gh issue create` direct, often retroactive); bugs (`gh` + `/triage`, skip spec); editorial refinement (`gh issue edit`, not `/triage`); single new issue not from plan. NOT upstream `implement`/`code-review` (deferred). | V-007 / [ADR-0007](docs/adr/0007-intake-pipeline-sequencing.md) + [ADR-0009](docs/adr/0009-wayfinder-decision-map-layer.md) |
