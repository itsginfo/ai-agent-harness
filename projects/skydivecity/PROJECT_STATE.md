...
> **Last updated:** 2026-06-18 by PM Agent (`#15` — Option B email SENT; verification call pending. Mirror-image model VALIDATED with live GA4 data (page_view dark→on / native events on→dark, crossover at 5-18). A-vs-B recommendation teed up, deferred to post-call per James. New scope `#16` — Omnyra AI, James meets Josh 2026-06-22)

---

## ⚡ RESUME INSTRUCTION

**Managed Services steady state. `#15` (Sev-3 GA4 incident) — Option B agreement email SENT (James, 2026-06-18); Marcella agreed + asked for a verification call (this afternoon / tomorrow).** Option B = BM removes GTM's GA4 config tag (+ GTM FB pixel) so Burble native owns GA4 again → restores pre-05-18 purchases/revenue/funnel-events + the GA4-imported "Burble Purchase" Ads conversion (Smart Bidding signal). All in BM's container, **no Burble change**.

**✅ GA4 data investigation COMPLETE — mirror-image model VALIDATED with live data (2026-06-18).** Marcella's reply questioned why SDC's booking page_views differ from her other Burble clients. Investigated end-to-end: (a) read the live native bundle (`bookings.burblesoft.com/assets/js/js_tracking/google_analytics_4.js`) → native fires **funnel events + ecommerce only, NO page_view** (auto OR manual); `send_page_view:false` confirmed in rendered config → **GTM is the sole page_view source**. (b) Built a GA4 free-form Explore ("Burble: page_views vs events", host-filtered to bookings.burblesoft.com) → **page_view dark Apr20→May17, on after; native `Add Jumper` alive→cliff-drop at May17; the two cross at 5-18.** Definitive proof the 5-18 fix *swapped which pipeline GA4 listened to* — call-ready visual. *(Interim hypothesis that pre-5-18 page_views existed was a CSV column misread — Active users read as Views — disproven by the Explore pull; original model stands.)*

**Analytical framing for the decision:** native events = the **outcome/revenue layer** (revenue, products, conversions, Ads/Smart-Bidding, step drop-off — irreplaceable); page_views = the **journey/engagement layer**. **Burble is ONLY the booking+payment bottom-funnel** (WP owns top-of-funnel + keeps its own page_views), so on Burble the page_view residual value shrinks to ~intra-step abandonment. ∴ if Marcella is event-based, **Option B preserves ~all decision-driving insight**; page_view loss is cheap.

**🔑 PIVOTAL OPEN Q (decides A vs B) — confirm on the call:** *does Marcella build her Burble funnel reports / audiences from the **native event names** (Add Jumper / Agreement / Payment / Confirmation Bookings) or from **page paths** (`/index/agreement`, etc.)?* Strong evidence she's **event-based** (her complaint = "revenue/data gone since 5-18" = the native signal that died; page_views actually *improved* at 5-18 yet she still reports a loss). If event-based → **B viable, not a compromise.** If path-based → need **A** (GTM carries page_views + events).

**A-vs-B recommendation is teed up but DEFERRED to post-call per James (2026-06-18).** James will follow up after meeting Marcella.

**Next:** (1) James meets Marcella (verification call, this week) — confirm the pivotal Q + walk the crossover chart. (2) Agent: deliver the A-vs-B recommendation once the pivot is known. (3) Then BM GTM change + end-to-end test booking. (4) Fold the May-report window correction into the June Digital Ops Report (~2026-07-05). Diagram + both prior BM replies reflect Option B; latest commit `2337bac`.

**NEW scope — `#16` (Omnyra AI integration, `needs-info`):** Rich connected James with Josh Caruso (Omnyra AI). Omnyra built SDC's AI receptionist; new branch for customer capture/retention into First Jump Course + AFF, to be integrated on the skydivecity.com domain — likely **replacing the existing AFF Google Form signup page** with Omnyra's hosted page/endpoint. **James meets Josh Mon 2026-06-22 for a walkthrough.** Pre-discovery; likely a Project SOW (see Pending Project SOWs → SCOPE-2).
...
|------|----------|---------|------|
| **#15 GA4 purchase-tracking incident** | Verification call with Marcella (this week; awaiting James's report-back) | [`skydivecity-com#15`](https://github.com/itsginfo/skydivecity-com/issues/15) | Sev 3. Option B email SENT; call pending. GA4 mirror-image model VALIDATED with live data (crossover chart, call-ready). **🔑 Pivotal Q for the call:** does Marcella build funnels/audiences from native event names or page paths? → decides A vs B. A-vs-B rec deferred to post-call per James. Then BM GTM change → test booking. |
| **#16 Omnyra AI integration walkthrough** | James ↔ Josh Caruso, Mon 2026-06-22 | [`skydivecity-com#16`](https://github.com/itsginfo/skydivecity-com/issues/16) | New scope (`needs-info`). Walkthrough of Omnyra's customer-capture/retention tool (FJC + AFF) → gather integration mechanics. Likely replaces the AFF Google Form signup with Omnyra's hosted page/endpoint. Pre-discovery → Project SOW (SCOPE-2). |
| **June Digital Ops Report** | Due ~2026-07-05 (SOW §4.1) | New issue at month-end | First clean full-month conversion baseline (May funnel was dark pre-2026-05-18). **Must include the facts-only May-report correction** (166/$68,298 was May 1–17 pre-fix, not post-fix — folded here per James 2026-06-13). Procedure: `reports/digital-ops-monthly/README.md`. |
...
- **GA4 purchase-tracking incident (`skydivecity-com#15`, incident, Sev 3) — root cause CONFIRMED 2026-06-12; BM walkthrough in progress.** Burble runs two pipelines; purchase event exists only in the native gtag one; our `#9` GTM fix (2026-05-18) caused GTM + native gtag to co-manage `G-FRW7R7G8EC` → native events silently swallowed (reproduced live via headless Chrome). Reply to Marcella **SENT 2026-06-13**. Remediation Option A (single GTM pipeline) recommended; needs BM to add GTM GA4 event tags for the full ecommerce set w/ UA→GA4 field mapping (pin `value`→`ga4_revenue`); we remove (not blank) Burble's GA4 key, then test-verify native stands down (empty key ≠ removed for `isGA4Enabled()`). Bonus: duplicate FB pixel inflating FB PageViews post-05-18. **2026-06-17: built interactive HTML walkthrough diagram** (`correspondence/2026-06-17-tracking-architecture-walkthrough.html`, `96c3df3`) for James's BM meeting. **NEW OPEN Q (gates diagram's Ads row):** Ads conversion counting — GTM tag (verified sole *page-fired* source, dark-before/live-after) vs GA4-import (would be alive-before/dark-after) vs both. Awaiting Marcella's answer → then correct diagram Ads row, keep verified parts. **Collateral: May report window misattribution → facts-only correction folded into June Digital Ops Report (~2026-07-05).** Awaiting BM on both the Ads-counting Q and remediation-coordination.
...
- **SCOPE-2: Omnyra AI customer-capture/retention integration ([`#16`](https://github.com/itsginfo/skydivecity-com/issues/16)).** Rich introduced **Josh Caruso (Omnyra AI)** on 2026-06-18. Omnyra built SDC's AI receptionist; new branch for customer capture + retention into the **First Jump Course + AFF**, to be integrated on the **skydivecity.com domain** — James's read is it **replaces the existing AFF Google Form signup page** with Omnyra's page/endpoint hosted on Omnyra's infrastructure. **James meets Josh Mon 2026-06-22** for a tool walkthrough to gather mechanics/logistics. Out of Managed Services scope; likely a Project SOW. Scope blocked on the walkthrough (`#16` = `needs-info`); once defined, `/to-prd` → `/to-issues`.
