...
**Managed Services stable. `#13` media-package pricing EXECUTED on prod 2026-06-11 + live-verified uncached** (5-phase CLEAN, 7 fields; edge cache cycled ~35 min): Hand Cam $125/$135, Outside $150/$160, Ultra-Premium $225/$235 (Cash/CC +$10 split kept per Rich). `#13` stays open on one checkbox: **confirm Matt applied Burble-side pricing**. **`#14` CLOSED not-planned** — Rich's screenshot was CSC's Burble page, not SDC's; we don't have "Display lowest prices available in calendar" enabled. Resurfaces with Dynamic Pricing rollout. **Dynamic Pricing Calendar: ON HOLD per Rich** — monitor media-price impact first; his sheet has red notes; when re-activated, include the price-inline-next-to-time preference (ex-`#14`).

**Next:** (a) Close `#13` once Matt's Burble-side change is confirmed. (b) Merch Discovery still ⛔ BLOCKED on Burble-tender Qs; unblocked branches available. (c) June report ~2026-07-05.
...
- **Burble timeslot price-inline (`skydivecity-com#14`) — ❌ CLOSED not-planned 2026-06-11.** Rich's reference screenshot was **CSC's** Burble page; SDC doesn't have "Display lowest prices available in calendar" enabled, so the stacked time/price layout doesn't exist on our funnel. Folded into the Dynamic Pricing item below as a rollout requirement.
...
1. **[Awaiting client] `#13` close-out** — confirm Matt applied Burble-side media pricing, then close. *(Website side ✅ executed + live-verified uncached. `#14` closed not-planned — was CSC's page, not ours.)*
