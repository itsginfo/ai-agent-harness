...
> **Last updated:** 2026-07-06 by PM Agent (Rich provided June Burble count → **net 507 tandem reservations** folded into June report §4 recorded-not-reconciled; PDF re-rendered; report now awaits James feedback → send. `#13` media pricing CLOSED — Burble-side confirmed. Prod queue EMPTY, no incidents. `#21` scoping + homepage-direction thread still open.)

---

## ⚡ RESUME INSTRUCTION

**Managed Services — no incidents. Prod queue EMPTY. `#13` CLOSED (Burble-side pricing confirmed 2026-07-06). June report unblocked — Rich's count folded in, awaits James's feedback → send. Remaining: `#21` scoping + homepage-direction/redesign thread.**

**✅ 4 PROD WRITES SHIPPED + LIVE-VERIFIED (2026-07-02 later-4).** Full 5-phase, SHA-verified bit-for-bit, all HTTP 200 + content + no error strings. All 4 issues closed. Commits `4337b5b` (slug fix + inventory), `38fb4f0`/`27d1730` (scripts).
- **`#18`** Flight-1 101/102 (Aaron Stocum, Jul 25–26) → prod event **5890** + image.
- **`#17`** Winterfest 2027 content → prod event **5865** (content + $99 early-bird + LOs + Burble reg link + banner).
- **`#19`** Home AFF CTA + Option A → page 2 band ("Sign Up for AFF"→academy button; "Learn More" repointed About→Learn-to-Skydive/1098; AFF hook **prepended** non-destructively, SEO block preserved).
- **`#22`** NEW 2026 USPA Collegiate Championship (Dec 28 2026–Jan 2 2027) → prod event **5894** + logo. *Open item: no reg URL (ships CTA-less).*

> **⚙️ Phase-1 inventory earned its keep:** post IDs diverge dev↔prod (2027 events created directly on prod 2026-05-11 got prod IDs; dev assigned different ones). `#17` hardcoded 5878 = a *revision* on prod; real event = 5865. Fixed the script to resolve by **slug** (`winterfest-2027`, POST_ID env override). **Lesson for future event UPDATES: resolve by slug, never hardcoded ID.** Core page IDs (page 2 = Home) ARE stable across dev/prod. Add to [[wp-acf-rendering]] / [[prod-write-procedure]].

**🎨 Homepage direction / redesign (James raised 2026-07-02).** `#19` was a tactical copy patch; the friction it hit (orphan ACF meta, a "body" field hiding a 5-para SEO block, dev/prod ID drift) is redesign signal. PM thoughts given: (1) Rich's "AFF buried" = IA problem, not copy; decide the homepage's #1 conversion goal explicitly. (2) Let the **clean July+ GA4 baseline** rank competing CTAs (tandem vs AFF vs events) rather than intuition. (3) Rationalize the content model in the redesign so copy edits stop being archaeology. (4) Preserve the SEO body copy as a migratable asset. (5) Don't let tactical patches become the de-facto redesign — run structured discovery from goals+data (Project-SOW-shaped; ties to paused `feature/redesign-phase2` + `#21` as a possible pilot). Open question it forces: **what is the "Best Skydiving School" band FOR?** (AFF-conversion → replace-line-1; brand/SEO anchor → prepend/skip).

**📤 June Digital Ops Report — Burble count FOLDED IN (`233b23b`), awaits James feedback → send.** Rich (2026-07-06): **net 507 tandem reservations** (519 gross − 12 cancellations). §4 records-not-reconciles it vs GA4's 83 (tracking dark Jun 1–21 → GA4 saw only the ≈9-day tail; July = first like-for-like month). PDF re-rendered. James sends (never agent).

**Next:** (1) June report — James reviews §4 framing → sends (To Rich, CC Matt). (2) `#21` Winterfest landing page — `needs-info`, needs scoping/PRD (possible redesign pilot per above). (3) `#19` body treatment — shipped as prepend; James retains editorial toggle (trivial follow-up). (4) When ready, open the homepage-direction/redesign discovery thread. *(`#13` closed 2026-07-06.)*

> **⚠️ ACF orphan-meta gotcha (home page, page 2)** — carries dead meta from prior layouts (`content_1_formatted_*`, `content_2_left_*`) that does NOT render; edit only the LIVE `content_3_*` keys and **verify against the rendered page**, not the DB. Add to [[wp-acf-rendering]] wiki.
...
|------|----------|---------|------|
| **June Digital Ops Report — SEND** | James review + send (was due ~2026-07-05; content done 2026-07-02) | commit `233b23b` | 📤 Content-complete + PDF re-rendered; **Burble count folded in — awaits James feedback → send.** §4: net 507 Burble tandem reservations recorded-not-reconciled vs GA4's 83 (tracking dark Jun 1–21; ≈9-day tail only); May correction carried; careers-page rec in §6. **July is** the first clean like-for-like month. `#13` refs updated to since-closed. |
...
1. **[Recurring, due ~2026-07-05] June Digital Ops Monthly Report** — first clean full-month baseline. **Must include:** (a) facts-only May-report correction (166/$68,298 = May 1–17 pre-fix); (b) recommendation to create a careers/employment page → `careers.skydivecity.com` (Omnyra subdomain DNS-live but unwired). Follow `reports/digital-ops-monthly/README.md`; file the issue at month-end.
2. **[Awaiting client] `#13` close-out** — confirm Matt applied Burble-side media pricing, then close. *(Website side ✅ executed + live-verified uncached. `#14` closed not-planned — was CSC's page, not ours.)*
3. **[In-flight, blocked + has unblocked branches] E-commerce Merch Discovery** — *James:* confirm the Burble-tender question with Rich/Matt (5 Qs in `ecommerce-merch-discovery.md`; crux = is Burble-balance merch tender hard-requirement or negotiable). *Agent:* resume `/grill-with-docs` on an unblocked branch (licensing/subscription costs → direct LOE input, or returns/exchanges). Toward PRD (task #6) + exec-summary-with-LOE (task #7).
4. **[Whenever bandwidth]** SCOPE-1 (Burble calendar) scope-gathering when Rich/Matt available; polish `#1`–`#3` (`#3` unfreezes `deploy.sh --live`). *(SSL `#4` closed 2026-06-01 — auto-renewal verified resolved; next renewal ~2026-07-07 needs no action.)*
