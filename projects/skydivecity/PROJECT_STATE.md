...
> **Last updated:** 2026-06-11 by PM Agent (Rich's media-pricing request → `#13` + `#14` filed; inventory done; dynamic pricing ON HOLD)

---

## ⚡ RESUME INSTRUCTION

**Managed Services stable. Two new Routine Requests from Rich's 2026-06-11 email:** **`#13`** media-package pricing (Handcam $105→$125, Outside $135→$150, Ultimate $205→$225; website side only — Burble side is Matt's). Inventory done dev-first + prod read-only (`migration/wp-media-pricing-inventory.php`, commit `d0b836c`): single rendered surface = `pricingtable` 5613 on `/skydiving-prices/` + `/media-packages/`. ⚠️ **Execution blocked (needs-info):** table has Cash/CC columns (+$10 spread); Rich gave one price each — keep split or collapse? **`#14`** Burble timeslot price-next-to-time CSS (cosmetic). **Dynamic Pricing Calendar: ON HOLD per Rich** — rollout media-price change first; sheet has red notes for later.

**Next:** (a) `#13` once Rich answers the CC question — 5-phase prod-write. (b) `#14` DOM inspect + CSS draft. (c) Merch Discovery still ⛔ BLOCKED on Burble-tender Qs; unblocked branches available. (d) June report ~2026-07-05.
...
- **Media-package pricing update (`skydivecity-com#13`, routine-request, needs-info)** — Rich 2026-06-11. Inventory complete + prod-verified: 6 postmeta values on `pricingtable` 5613 (Cash/CC × 3 rows), rendered on `/skydiving-prices/` + `/media-packages/`; `bookinglink` 5508 ("Starting at $105") is orphaned (optional consistency fix). Awaiting Rich/Matt: Credit-Card column treatment. Matt owns Burble-side pricing — confirm both land together.
- **Burble timeslot price-inline (`skydivecity-com#14`, routine-request)** — price next to time instead of below in the PICK TIME slot buttons. Same surface as `#7`/`#12` (`tandem-bookings-page-styles.css` + Burble admin CSS panel); scope selectors tightly (lesson from `#12`).
- **Dynamic Pricing Calendar — ON HOLD per Rich 2026-06-11** — proceeding eventually, but NOT with time-of-day variations yet; wants to monitor media-price impact first. Rich updated the shared sheet (notes in red) — pick up there when re-activated.
...
1. **[New, needs-info] `#13` media-package pricing** — *Rich/Matt:* answer the Cash/CC-column question (keep +$10 split or collapse?). *Agent:* on answer, 5-phase prod-write on `pricingtable` 5613 + verify rendered HTML on both pages; coordinate timing with Matt's Burble-side change. Then `#14` (Burble timeslot CSS — DOM inspect first).
2. **[In-flight, blocked + has unblocked branches] E-commerce Merch Discovery** — *James:* confirm the Burble-tender question with Rich/Matt (5 Qs in `ecommerce-merch-discovery.md`; crux = is Burble-balance merch tender hard-requirement or negotiable). *Agent:* resume `/grill-with-docs` on an unblocked branch (licensing/subscription costs → direct LOE input, or returns/exchanges). Toward PRD (task #6) + exec-summary-with-LOE (task #7).
3. **[Recurring, due ~2026-07-05] June Digital Ops Monthly Report** — first clean full-month conversion baseline (May funnel was dark pre-2026-05-18). Follow `reports/digital-ops-monthly/README.md`; file the issue at month-end. *(May report `#10` ✅ delivered 2026-06-02.)*
