...
|------|----------|---------|------|
| **#15 GA4 purchase-tracking incident** | BM walkthrough 2026-06-17 (diagram built); awaiting Marcella on Ads conversion-counting mechanism + BM remediation-coordination | [`skydivecity-com#15`](https://github.com/itsginfo/skydivecity-com/issues/15) | Sev 3. Option A recommended. Open Q: Ads counts via GTM tag / GA4 import / both → determines diagram Ads-row correction. Next: Marcella's answer → fix diagram → BM agrees option → Burble + GTM changes → test booking. |
...
1. **[ACTIVE INCIDENT, awaiting Marcella] `#15` GA4 purchase tracking** — James is walking BM through the HTML diagram. *On report-back:* update the diagram's Ads row per Marcella's answer on conversion-counting (GTM tag / GA4 import / both), keeping verified parts untouched. Then coordinate remediation Option A with BM → implement + test booking. Corroborate in GA4 UI (native custom events zero since 05-18). May-report window correction folded into June report.
