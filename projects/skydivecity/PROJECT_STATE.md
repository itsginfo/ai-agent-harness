...
> **Last updated:** 2026-06-13 by CTO Agent (`#15` — BM reply SENT after native-bundle verification; Option A tightened to full event-set + revenue pinning + blank-key check; awaiting BM remediation response)

---

## ⚡ RESUME INSTRUCTION

**ACTIVE INCIDENT `#15` (Sev 3): GA4 purchase/revenue events dark since 2026-05-18 — caused by our `#9` GTM fix.** Root cause CONFIRMED via live-page repro: GTM loading + Burble's native gtag both managing `G-FRW7R7G8EC` → native events (incl. `purchase`, the ONLY purchase source) pushed to dataLayer but never transmitted. Pre-05-18 era was the mirror image (purchases alive via native pipeline, pageviews/Ads dark). **`#9` Defect-2 "dead code" verdict was wrong; May report's "post-fix window" attribution was wrong** (166/$68,298 was May 1–17) — facts-only correction to Rich pending. Reply to Marcella (Beyond Marketing) **SENT 2026-06-13** (`correspondence/2026-06-12-beyond-marketing-ga4-transactions-reply.md`); recommends Option A (single GTM pipeline), tightened pre-send via native-bundle verification: forward the **full** ecommerce set (`add_to_cart`/`begin_checkout`/`remove_from_cart`/`purchase`, not just `purchase`), pin GA4 `value`→`ga4_revenue` + `transaction_id` + `items[]`, and test-verify the native pipeline stands down after blanking (`isGA4Enabled()` keys on `typeof string` → empty ≠ removed). **Now awaiting BM remediation-coordination response** (GTM container is theirs). Verified collateral: conversion IDs live in BOTH Burble's form and GTM; Burble native fires NO Ads conversion (GTM `/index/finish/` tag is sole source). May report's "post-fix window" attribution correction (166/$68,298 was May 1–17, pre-fix) is **folded into the June Digital Ops Report** (~2026-07-05) per James 2026-06-13 — not a standalone note. Full evidence + task list on `#15`.

**Also open:** `#13` awaits Matt's Burble-side pricing confirmation. Merch Discovery ⛔ BLOCKED (unblocked branches available). June report ~2026-07-05. Dynamic Pricing ON HOLD per Rich (carry-along: price-next-to-time, ex-`#14`).

**Backlog if no bandwidth:** SCOPE-1 awaits Rich/Matt; polish `#1`–`#3`; harness `ai-agent-harness#1/#3/#5`.
...
|------|----------|---------|------|
| **#15 GA4 purchase-tracking incident** | BM reply sent 2026-06-13 — awaiting BM remediation-coordination response | [`skydivecity-com#15`](https://github.com/itsginfo/skydivecity-com/issues/15) | Sev 3. Option A (single GTM pipeline) recommended; reply sent. Next: BM agrees option → Burble + GTM changes → test booking verifies. |
...
- **GA4 purchase-tracking incident (`skydivecity-com#15`, incident, Sev 3) — root cause CONFIRMED 2026-06-12.** Burble runs two pipelines; purchase event exists only in the native gtag one; our `#9` GTM fix (2026-05-18) caused GTM + native gtag to co-manage `G-FRW7R7G8EC` → native events silently swallowed (reproduced live via headless Chrome, consent accepted: only GTM `page_view` transmits). Reply to Marcella **SENT 2026-06-13** (`correspondence/2026-06-12-...md`). Remediation Option A (single GTM pipeline) recommended; needs BM to add GTM GA4 event tags for the full ecommerce set (`add_to_cart`/`begin_checkout`/`remove_from_cart`/`purchase`) w/ UA→GA4 field mapping (pin `value`→`ga4_revenue`); we blank Burble's GA4 key, then test-verify the native pipeline stands down (empty key ≠ removed key for `isGA4Enabled()`). Bonus: duplicate FB pixel (GTM + native) inflating FB PageViews post-05-18. **Collateral: May report window misattribution → facts-only correction folded into the June Digital Ops Report (~2026-07-05) per James 2026-06-13.** **Now awaiting BM remediation-coordination response.**
...
1. **[ACTIVE INCIDENT, awaiting James] `#15` GA4 purchase tracking** — James reviews + sends drafted reply to Marcella; then coordinate remediation option with BM; then implement + test booking. Also: corroborate in GA4 UI (native custom events should be zero since 05-18) and correct May-report window with Rich. Wiki updates (`burble-integration.md`, `tracking-stack.md` Defect-2 reversal) ride along.
...
4. **[Recurring, due ~2026-07-05] June Digital Ops Monthly Report** — first clean full-month conversion baseline (May funnel was dark pre-2026-05-18). Follow `reports/digital-ops-monthly/README.md`; file the issue at month-end. *(May report `#10` ✅ delivered 2026-06-02.)*
