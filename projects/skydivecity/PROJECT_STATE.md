...
> **Last updated:** 2026-07-02 (later 2) by PM Agent (PAUSED mid-session per James. THREE prod writes dev-built + queued (`#18`/`#17`/`#19`) BLOCKED on Flywheel SSH; `#19` copy-enhancement options proposed, awaiting James's pick. June report paused for Burble count.)

---

## ⚡ RESUME INSTRUCTION

**Managed Services — no incidents. THREE prod writes staged + BLOCKED on Flywheel SSH; a copy decision + a report paused on James/client.**

**🚧 BLOCKER — Flywheel SSH gateway outage (2026-07-02).** `kex_exchange_identification: Connection reset by peer` (port 22 accepts TCP then resets mid-handshake). **Prod site HTTP 200 (unaffected); SSH-only.** Likely the **automated-throttle** pattern (James: prior block was Flywheel auto-security on too many connections) — **not** confirmed a gateway outage. Plan agreed: **wait ~1hr cooldown (started ~2026-07-02 mid-session), then re-probe with a SINGLE `ssh … echo SSH_OK`** (do NOT loop-probe — that reinforces the throttle). If still blocked after cooldown, escalate to Flywheel Sr Eng (James). **On recovery, run the 3 queued prod writes in order (#18 → #17 → #19).**

**⏸️ THREE dev-built prod writes QUEUED** (each: `scp -O` script + PNG to `/tmp`, Phase-1 inventory, execute, verify, close):
- **`#18` Flight-1 event** (Jul 25–26, ~3 wks out — most urgent). Script `migration/wp-event-add-flight1-jul2026.php` + `flight1-canopy-courses-jul25-26.png`. Dev 5881. **James approved for prod.**
...
- **`#19` home AFF CTA** ("Sign Up for AFF"→academy in the "Best Skydiving School" band, section-3 real ACF keys). Script `migration/wp-home-aff-cta-2026-07.php` (`131d4a1`). Dev-verified.

**⏳ `#19` copy enhancement — AWAITING JAMES'S PICK.** He asked for AFF-prominence quick-wins in the "FLORIDA'S PREMIER SKYDIVING CENTER"/"Best Skydiving School" band (copy-only, no redesign). Proposed 3 options (CMO lens); **recommended Option A** (reorient band body copy to AFF: *"Ready to earn your wings? Our AFF program takes you from student to USPA-licensed skydiver…"*) **+ repoint the band's "Learn More" button to the AFF info page (1098)**. On his pick → build on dev (band = section-3 `content_3_left_left_bb_0_text_*` + button link), verify, then it joins the `#19` prod write.
...
**Next (once SSH up):** (1) 3 queued prod writes `#18`→`#17`→`#19` (fold in `#19` copy if picked). (2) `#20` Nationals CP + `#21` Winterfest landing page — `needs-info` (‹#20› logo/event mismatch, Rich queried; ‹#21› have Summerfest URL, needs scoping/PRD). (3) June report send. (4) `#13` close-out awaits Matt.
...
|------|----------|---------|------|
| **🚧 Flywheel SSH outage** | Re-probe each session until restored; James escalate to Flywheel Sr Eng | [[flywheel]] | Started 2026-07-02. Key-exchange reset; site HTTP 200 (SSH-only). **Blocks all prod DB writes** — `#18`/`#17` queued. Clears when `ssh … echo SSH_OK` succeeds. |
| **`#18`/`#17`/`#19` — prod push** | On SSH recovery | [`#18`](https://github.com/itsginfo/skydivecity-com/issues/18) · [`#17`](https://github.com/itsginfo/skydivecity-com/issues/17) · [`#19`](https://github.com/itsginfo/skydivecity-com/issues/19) | All 3 dev-built + verified; `#18` approved. Flight-1 ~3 wks out (most urgent); Winterfest early-bird ends Jul 31; `#19` AFF band button. Scripts staged; 5-phase queued. `#19` may also carry a band copy enhancement (awaiting James's pick). |
