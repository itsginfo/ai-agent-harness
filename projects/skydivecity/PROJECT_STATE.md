...
> **Last updated:** 2026-07-02 (later 3) by PM Agent (SSH RESTORED — blocker cleared. FOUR prod writes now dev-built + queued (`#18`/`#17`/`#19`/`#22`), HELD for James's go. `#19` Option A dev-built non-destructively; body treatment awaiting James. `#20` logo ambiguity resolved → new collegiate event `#22`. June report still paused for Burble count.)

---

## ⚡ RESUME INSTRUCTION

**Managed Services — no incidents. SSH BACK; FOUR prod writes staged + ready, HELD for James's go. One `#19` editorial pick + a report paused on James/client.**

**✅ SSH RESTORED (2026-07-02 later-3).** Single non-looping probe `ssh … echo SSH_OK` → `SSH_OK`, rc=0. The Flywheel throttle/outage cleared. Prod-write blocker is GONE. (Host `ssh.getflywheel.com:22`, user `team+beyondmarketing+skydive-city`, key `~/.ssh/id_ed25519_flywheel`, sourced from `.env.deploy`.)

**⏸️ FOUR dev-built prod writes QUEUED — HELD for James's go** (each: `scp -O` script + PNG to `/tmp`, Phase-1 inventory, execute, verify, close). *PM held rather than run unsupervised: James stepped away mid-decision, `#19` has an open editorial pick, `#22` not yet prod-approved, and this is right after a throttle incident. Items have weeks of runway.* Run order:
- **`#18` Flight-1 event** (Jul 25–26, ~3 wks out — most urgent). Script `migration/wp-event-add-flight1-jul2026.php` + `flight1-canopy-courses-jul25-26.png`. Dev 5881. **James approved for prod — ready to go on his word.**
- **`#17` Winterfest content** (event 5878). Script `migration/wp-event-update-winterfest-2026-07.php` + `winterfest-banner-wide.png`. DRY_RUN-guarded. Early-bird ends Jul 31.
- **`#19` home AFF CTA + Option A** (section-3 band). Script `migration/wp-home-aff-cta-2026-07.php` (`38fb4f0`). Dev-verified. **⏳ pending James's body-treatment pick (see below).**
- **`#22` NEW 2026 USPA Collegiate Championship event** (Dec 28 2026–Jan 2 2027). Script `migration/wp-event-add-collegiate-2026.php` (`27d1730`) + `uspa-collegiate-championships-2026-logo.png`. Dev 5885, verified + idempotent. Open item: no reg/booking URL supplied (ships CTA-less, matches 2024 ref).

**⏳ `#19` Option A — dev-built, but body treatment AWAITING JAMES'S PICK.** James picked Option A. On build, found the "band body copy" is NOT a throwaway line — it's a live 5-para SEO/brand block (30yrs / 80 countries / Z-Hills + internal links). Literal replace would delete it. Applied **non-destructively on dev**: (1) **prepended** the AFF hook (SEO block preserved), (2) repointed "Learn More" button About(1003)→Learn-to-Skydive(1098). "Sign Up for AFF"→academy button already in base. **James to confirm body treatment** (one field, trivially switchable pre-push): (1) prepend [current], (2) replace line-1 only, (3) skip body. Commented on `#19`.
...
**Next:** (1) On James's go — run 4 queued prod writes `#18`→`#17`→`#19`→`#22` (confirm `#19` body pick + `#22` prod-approval first). (2) `#20` Nationals CP (5761) — logo ambiguity RESOLVED (collegiate logo → new `#22`, not 5761); `needs-info` now = "does 5761 need any update at all, or close?" (asked James). `#21` Winterfest landing page — `needs-info`, needs scoping/PRD. (3) June report send. (4) `#13` close-out awaits Matt.
...
|------|----------|---------|------|
| **`#18`/`#17`/`#19`/`#22` — prod push** | On James's go (SSH is up) | [`#18`](https://github.com/itsginfo/skydivecity-com/issues/18) · [`#17`](https://github.com/itsginfo/skydivecity-com/issues/17) · [`#19`](https://github.com/itsginfo/skydivecity-com/issues/19) · [`#22`](https://github.com/itsginfo/skydivecity-com/issues/22) | All 4 dev-built + verified; SSH restored. **HELD for James's go** (unsupervised prod writes not run: `#19` body pick open, `#22` not prod-approved, post-throttle caution). `#18` approved. Flight-1 ~3 wks out; Winterfest early-bird ends Jul 31; `#19` AFF band (body pick pending); `#22` new Collegiate event (no reg URL). 5-phase per item. |
...
> *Retired 2026-07-02 (later 3):* "🚧 Flywheel SSH outage" row — ✅ RESOLVED. Single non-looping probe returned `SSH_OK` (rc=0); throttle/outage cleared. Prod-write blocker gone; the prod-push row above now tracks the (now unblocked) queue, held for James's go.
