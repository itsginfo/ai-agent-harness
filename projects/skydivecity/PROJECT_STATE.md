...
> **Last updated:** 2026-07-02 (later) by PM Agent (New content intake from Rich → 5 issues `#17`–`#21`. `#18` Flight-1 + `#17` Winterfest **dev-built + verified**, prod writes BLOCKED on a Flywheel SSH gateway outage. June report still paused for Burble count.)

---

## ⚡ RESUME INSTRUCTION

**Managed Services — no incidents. TWO prod writes staged + BLOCKED on Flywheel SSH; one report paused on client data.**

**🚧 BLOCKER — Flywheel SSH gateway outage (2026-07-02).** `kex_exchange_identification: Connection reset by peer` — port 22 accepts TCP then resets mid-handshake. **Production site is HTTP 200 (unaffected); SSH-surface only.** Per [[flywheel]], escalate to a Flywheel Sr. Engineer via chat (L1 can't diagnose this class) — **James's action.** Blocks ALL prod DB writes (`wp eval-file` over SSH). **First thing next session: re-probe SSH; if up, run the two queued prod writes below.**

**⏸️ `#18` Flight-1 event (Jul 25–26 2026) — DEV-BUILT + APPROVED, prod write QUEUED.** Script `migration/wp-event-add-flight1-jul2026.php` (`25f4f80`), dev event 5881 verified. James approved for prod 2026-07-02. **Most time-sensitive (event ~3 wks out).** On SSH recovery → 5-phase: `scp -O` script + `flight1-canopy-courses-jul25-26.png` to `/tmp`, Phase-1 inventory (slug `flight1-aaron-stocum-jul26` free? cats exist?), execute, verify live, close `#18`.

**⏸️ `#17` Winterfest content (event 5878) — DEV-BUILT + verified, prod write QUEUED (behind #18).** Script `migration/wp-event-update-winterfest-2026-07.php` (`b4d9abc`), DRY_RUN-guarded. Needs `winterfest-banner-wide.png` scp'd to `/tmp`. Early-bird ends Jul 31 → timely. (James: review dev render http://localhost:8080/event/winterfest-2027/ + confirm featured-image choice before prod.)

**⏸️ June Digital Ops Report — CONTENT-COMPLETE (`d510783`), delivery PAUSED** for Rich's June Burble count → James feedback → James sends. §4: tracking dark Jun 1–21, restored ~Jun 22–23; 83/$32,476 tail-only; May-figure correction carried; reconciliation deferred to July.

**Next (once SSH up):** (1) prod-push `#18` then `#17`. (2) `#19` home-page AFF visibility — dev build (propose placement first). (3) `#20` USPA Nationals CP + `#21` Winterfest landing page — both `needs-info` (‹#20›: logo/event mismatch, Rich queried; ‹#21›: have Summerfest URL https://skydivechicago.com/summerfest/, needs scoping/PRD). (4) June report send. (5) `#13` close-out awaits Matt.
...
| **`#18` Flight-1 + `#17` Winterfest — prod push** | On SSH recovery | [`#18`](https://github.com/itsginfo/skydivecity-com/issues/18) · [`#17`](https://github.com/itsginfo/skydivecity-com/issues/17) | Both dev-built + verified; `#18` approved. Flight-1 event ~3 wks out (most urgent); Winterfest early-bird ends Jul 31. Scripts staged; 5-phase queued. |
