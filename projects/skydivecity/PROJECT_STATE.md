...
**Managed Services steady state. Top item `#15` (Sev-3 GA4 incident) is EXTERNALLY GATED — nothing agent-actionable until Beyond Marketing replies.** Root cause confirmed + reply SENT 2026-06-13 (Option A, single GTM pipeline; BM owns the container). When BM responds: agree option → remove (not blank) Burble's GA4 key + test-verify native pipeline stands down → BM adds GTM GA4 event tags for the full ecommerce set (pin `value`→`ga4_revenue`) → end-to-end test booking → fold in duplicate-FB-pixel cleanup. Evidence, tightened plan, and verified facts (IDs in BOTH form+GTM; native fires no Ads conversion) all on `#15`; wiki [[burble-integration]]/[[tracking-stack]] updated.

**Also open:** `#13` awaits Matt's Burble-side pricing confirmation (website side done). **June Digital Ops Report due ~2026-07-05 — must include the facts-only May-report correction** (166/$68,298 was May 1–17 pre-fix, not post-fix). Merch Discovery ⛔ BLOCKED on Burble-tender question (unblocked branches available). Dynamic Pricing ON HOLD per Rich (carry-along: price-next-to-time, ex-`#14`).

**Next (proactive — `#15` is BM-gated):** resume Merch Discovery on an unblocked branch (licensing/returns/accounts) toward PRD, or polish `#1`–`#3` (`#3` unfreezes `deploy.sh --live`). Backlog: SCOPE-1 awaits Rich/Matt; harness `ai-agent-harness#1/#3/#5`.
