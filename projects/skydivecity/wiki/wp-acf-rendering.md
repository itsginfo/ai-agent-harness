...
- **Build-time inlining has a ceiling — it is right for a handful of brand marks, wrong for a photo roster** (measured 2026-09-02, `#42`). The Winterfest template inlines four brand assets as base64 data-URIs and sits at ~790KB. Twenty-four load-organizer portraits would add roughly **670KB of base64**, producing a ~1.5MB template parsed on every page load, with **no lazy loading, no `srcset`, and no per-image caching** — and a photo could not be replaced without a rebuild and redeploy. **Photo sets belong in the Media Library, referenced by URL**; keep inlining for the small fixed set of brand marks. Format matters more than treatment at roster scale: a 400px cut-out portrait is **198KB as PNG but 22KB as WebP** (4.7MB vs 528KB across 24), so transparency must ship as WebP, never PNG.
- **A dark landing page cannot absorb a light-background photo by CSS alone** (2026-09-02, `#42`). Radial fades only soften the rim while the background still fills the frame; monotone/duotone integrates but erases the individuality that justified the photo. The only treatment that is both seamless *and* recognizable is **background removal**. Decide a roster treatment against the *worst* source photos, never the best — mixed backgrounds are what break a treatment when the images are lined up together.
