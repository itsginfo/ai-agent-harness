...
- **⚠️ Keep remote commands short — one `wp` command per SSH session** (2026-09-14, `#46`/`#23`). Long compound remote commands (several `wp` calls + `$(...)` in one `ssh … '…'`) die with bare exit 255 and no output even while a plain `echo` succeeds. Upload a script with `scp -O` and `eval-file` it instead. Same session also saw the **gateway throttle** (auth OK, session closed before the command ran) for ~15 min after ~15 connections — third occurrence; it recovers unaided. Add `-o ServerAliveInterval=30` for multi-minute batches.
- **WP-CLI is NOT in the local Docker container** (2026-09-14). Use a phar: `docker exec skydivecity_wordpress sh -c 'curl -s -o /tmp/wp-cli.phar https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar'` then `docker exec skydivecity_wordpress php /tmp/wp-cli.phar --allow-root --path=/var/www/html <cmd>`. Table prefix on dev is `wp_vu3scguxbw_`.
- **Edge cache is Cloudflare's** (DNS is SDC-owned at Cloudflare; James has access — 2026-09-14). `wp cache flush` does not touch it; purge from the Cloudflare dashboard/API after prod content writes, or wait ~35 min.
